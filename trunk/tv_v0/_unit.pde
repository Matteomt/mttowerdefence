class _unit {
  ...
  void hit(float strength,_gun gun) {
    this.damage+=strength;
    if(this.position.start != 0)this.position.start+=gun.weight*gun.delay/10;
    if(damage>=life){
      life=0;
      damage=SPACE.time;
      this.position.motion_add( 0, 0, 0,
                                0, 0, max(0-this.position.z,-100.)/sq(DEBRISLIFE)
                               );        
      if(this.enemy)
        SPACE.enemys--;
      pushStyle();
      fill(0,0,100,100);
      noStroke();
      SPACE.d_ellipse(this.position,5*this.radius,5*this.radius);
      popStyle();
    }
    
    SPACE.units[gun.unit.id].tower_flags[0]++;
  ...
  void draw() {
    ...
    // --- ENEMY--------------------------
      ...
      switch(this.type) {
        default:
          if(this.life<=0)
            fill(0,100,100,SPACE.alternateFrame?(100-100*sq((SPACE.time-this.damage)/DEBRISLIFE)):0);
          else
            fill(0,100,100,SPACE.alternateFrame?(100-100*this.damage/this.life):100);
      }
    ...
    // --- TOWER--------------------------
      ...
      switch(this.type) {
        default:
          if(this.life<=0)
            fill(120, 20, 100,SPACE.alternateFrame?(100-100*sq((SPACE.time-this.damage)/DEBRISLIFE)):0);
          else
            fill(120, 20, 100,SPACE.alternateFrame?(100-100*this.damage/this.life):100);
      }
      SPACE.d_ellipse(this.position, 2*this.radius, 2*this.radius);
